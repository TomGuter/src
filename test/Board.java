...
public class Board {
	private static Board boardReference = null; // Static variable to hold the singleton instance
	private Tile[][] tiles;
	public int wordsCounter = 0;
	private ArrayList<Word> allNewWords = new ArrayList<>(); // use it as a kind of que for all the words that can be
																// created by the same turn
	...
	public static Board getBoard() {
		if (boardReference == null) {
			boardReference = new Board();
		}
		return boardReference;
	}

	public Tile[][] getTiles() {
		Tile[][] copyTiles = new Tile[15][15]; // Create a deep copy of the tiles array to prevent modification outside
												// the class
		for (int i = 0; i < 15; i++) {
	...
	public int tryPlaceWord(Word word) {
		...
		for (Word w : allNewWords) {
			scores += getScore(w); // calculate the total score for all created words by the same turn
		}
		RemoveCurrWords();
		return scores;
