...
public class Board {
	private static Board bagReference = null; // Static variable to hold the singleton instance
	private Tile[][] tiles;
	public int wordsCounter = 0;
	ArrayList<Word> allNewWords = new ArrayList<>(); // use it as a kind of que for all the words that can be created by the same turn
	...
	public static Board getBoard() {
		if(bagReference == null) {
			bagReference = new Board();
		}
		return bagReference;
	}

	public Tile[][] getTiles() {
		Tile[][] copyTiles = new Tile[15][15];   // Create a deep copy of the tiles array to prevent modification outside the class
		for(int i = 0 ; i < 15 ; i++) {
			System.arraycopy(tiles[i], 0, copyTiles[i], 0, 15);
			///copyTiles[i] = Arrays.copyOf(tiles[i], 15);
	...
	public boolean boardLegal(Word word) { // check if the board is currently legal to place the new word
		...
		if (!restsOnExistingTile(word) && wordsCounter > 0) { // the first if it passed the previous conditions word must be legal
			return false;
		}
		if (isRequiredReplacement(word)) {// if the new word requires a replacement of an existing tile on the board
			return  false;
		}
	...
	private boolean isInsideBoard(Word word) {
		...
		if ((word.isVertical()) && ((word.getRow() + word.getTiles().length - 1) > 14 || (word.getRow() + word.getTiles().length) < 0)) {
			return false;
		}
		if((!word.isVertical()) && ((word.getCol() + word.getTiles().length - 1) > 14 || (word.getCol() + word.getTiles().length) < 0)) {
			return false;
		}
		return  true;
	}
	...
	private boolean restsOnExistingTile(Word word) {
		...
		if (word.isVertical()) { // check the relevant situations, if the word is vertical or not
			while ((tempTiles.length + firstRow) > row) {
				...
				else if (i < tempTiles.length && tempTiles[i] == null && tiles[row][col] == null) { // if the current tile is null, so there is no need to check it
					i++;
					row++;
					continue;
				}
	...
	public ArrayList<Word> getWords(Word word) { // return an updated array with the new created words by the same turn

		if (!dictionaryLegal(word)) {
			return allNewWords; // Word is not legal according to the dictionary
		}
	...
	public int getScore(Word word) {
		...
		for (int i = 0 ; i < word.getTiles().length ; i++) {
			...
			if (tiles[row][col] != null) {
				currScore += (tiles[row][col].score * checkLetterBonus(row, col));
			}
		...
		totalWordScore += (currScore * wordBonus);
	...
	public int tryPlaceWord(Word word) {
		...
		if (!boardLegal(word)) {
            ...
            for (Tile wordTile : wordTiles) {
                ...
                if (wordTile != null && (!vertical) && ((row > 0 && tiles[row - 1][col] != null) || ((row < 14) && (tiles[row + 1][col] != null)))) {
                    createNewVerticalWord(word, row, col); // create a new word to the dictionary and calc the new score
                }
                if (wordTile != null && (vertical) && ((col > 0 && tiles[row][col - 1] != null) || ((col < 14) && (tiles[row][col + 1] != null)))) {
        ...
        for (Word w : allNewWords) {
            scores += getScore(w); // calculate the total score of all created words by the same turn
        }
		RemoveCurrWords();
		return scores;
	...
	private void createNewNotVerticalWord(Word word, int row, int col) {
		//for the potential case that the word is vertical
		int calcSize = 0;
		int tempCol = col;
		int startIndex = 0;
		if (row < 0 || row > 14 || col < 0 || col > 14) {
